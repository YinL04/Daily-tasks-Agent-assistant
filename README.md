...
1. 先说明业务目标：把自然语言目标转成任务、日历、资料入口和可下载计划。
2. 再讲上下文工程：输入、时间、记忆、Skill 定义和 LLM 状态如何被组装进一次 Run。
3. 然后讲 Agent Harness：为什么要限制步数、记录日志、标记 LLM/本地步骤。
4. 接着讲可降级设计：LLM 可用时增强智能，不可用时主流程仍能跑通。
5. 最后讲工程可扩展性：新增 Skill、替换 Provider、接入真实数据库或日历系统都不需要推翻主架构。
