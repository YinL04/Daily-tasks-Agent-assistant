...
import { CalendarPlus, ChevronLeft, ChevronRight, Download, Save, Trash2, Upload, X } from "lucide-react";
...
type DisplayCalendarEvent = CalendarEvent &
  Partial<Pick<StoredCalendarEvent, "source" | "sourceRunId" | "createdAt" | "updatedAt">>;
...
export default function CalendarView({
  ...
  async function importIcs() {
    if (!icsDraft.trim()) return;
    setError("");
    try {
      await calendarApi.importIcs(icsDraft);
      setIcsDraft("");
      await load();
    } catch (err) {
    ...
    <section className="panel calendar-panel">
      <div className="calendar-toolbar">
        ...
        <div className="calendar-actions">
          <button className="icon-button" onClick={() => shiftWeek(-1)} title="上一周">
            <ChevronLeft size={18} />
          </button>
          <button className="today-button" onClick={() => setWeekStart(startOfWeek(new Date()))}>
            本周
          </button>
          <button className="icon-button" onClick={() => shiftWeek(1)} title="下一周">
            <ChevronRight size={18} />
          </button>
          {!readOnly && (
            <a className="icon-button" href={calendarApi.exportUrl} title="导出 ICS">
              <Download size={18} />
            </a>
          )}
          {!readOnly && (
            <button className="primary-button compact" onClick={() => createOn(new Date())}>
              <CalendarPlus size={17} /> 新增
            </button>
          )}
        </div>
      </div>
      {!readOnly && (
        <div className="ics-import">
          <textarea
            placeholder="粘贴 .ics 内容后导入外部日历"
            value={icsDraft}
            onChange={(event) => setIcsDraft(event.target.value)}
          />
          <button onClick={() => void importIcs()}>
            <Upload size={15} /> 导入 ICS
          </button>
        </div>
      )}
            ...
            <div className="day-column" key={day.toISOString()}>
              <div className="day-head">
                ...
                {readOnly ? (
                  ...
                  <button onClick={() => createOn(day)} title="添加到这一天">
                    {day.getMonth() + 1}/{day.getDate()}
                  </button>
                )}
              </div>
      ...
      {draft && (
        <div className="calendar-editor">
          ...
          <div className="editor-actions">
            {draft.id && (
              <button className="danger-button" onClick={() => remove(draft.id!)}>
                <Trash2 size={16} /> 删除
              </button>
            )}
