# Agent 技术文档：这个项目是怎么工作的

这份文档用尽量直白的方式解释 `Personal Agent Planner` 这个项目。它适合刚开始接触 Agent、Prompt 工程、上下文工程、Harness、记忆系统和 Skills 设计的人阅读。

先把核心点说清楚：当前代码实现的是 **经典 ReAct loop**。

- 模型在每一轮根据当前状态和上一轮 Observation 生成 Thought。
- 模型被要求从当前可用工具里选择一个 Action；后端只接受 `allowedActions` 中的 Action。
- `AgentHarness` 负责限制、包裹执行，并把工具输出或错误记录为 Observation。
- Observation 会进入下一轮 prompt，让模型继续决定下一步，而不是由代码写死完整业务顺序。

你可以先把这个项目理解成一句话：

> 用户输入一个自然语言目标，系统尝试把它拆成任务、日历、网址、建议和文件，并且记录整个 Agent 是如何一步步完成这件事的。

## 0. v2 更新概览

v2 把项目从“一次性 Agent Run 工作台”升级成了“会话式个人事务 Agent + 个人计划资产层”：

- 新增 `ConversationStore`，会话和消息保存在 `data/conversations.json`。
- 每条用户消息都会写入当前会话，Agent 完成后会把 assistant 消息和完整 `runResult` 写回同一会话。
- `AgentContext` 增加 `conversationHistory` 和 `conversationSummary`，让第二轮、第三轮对话可以引用前文。
- `POST /api/agent/run` 支持流式响应，前端可以实时收到 `step`、`observation`、`partial`、`done` 和 `error`。
- `AgentHarness` 支持事件回调和 abort signal，前端停止按钮可以中断正在运行的 Agent。
- 修复了流式路由误判中断的问题：后端不再把正常请求关闭当作用户取消。
- 前端布局调整为左侧会话列表 + LLM 状态小框 + 工作台二级菜单；日历收进 `工作台 -> 日历`，不再挤占对话首屏。
- 安全层补充了输入长度限制、可选 `AUTH_TOKEN`、文件下载白名单、健康端点脱敏和 Agent Run rate limit。
- 记忆系统升级为 pending、active、archived 状态机，并记录 layer、hitCount、lastHitAt。
- `MemoryManager.retrieveRelevant()` 支持 LLM Top-K 记忆筛选，失败时降级到关键词打分。
- 新增场景模板、长期目标、周期复盘和生成文件清理，让 Agent 输出能进入长期维护。
- 新增 ICS 日历导入导出、Docker/Compose、GitHub Actions、ESLint/Prettier 和 SQLite migration SQL 导出。

核心代码入口是：

- `apps/server/src/agent/executor.ts`
- `apps/server/src/agent/context.ts`
- `apps/server/src/agent/harness.ts`
- `apps/server/src/storage/conversationStore.ts`
- `apps/server/src/routes/conversation.routes.ts`
- `apps/server/src/routes/agent.routes.ts`
- `apps/server/src/routes/template.routes.ts`
- `apps/server/src/routes/goal.routes.ts`
- `apps/server/src/routes/review.routes.ts`
- `apps/server/src/skills/*.skill.ts`
- `apps/server/src/skills/definitions/*.skill.md`
- `apps/server/src/memory/*`

## 1. 一张图看懂整体流程

```mermaid
flowchart TD
  A["用户输入：下周学习计划 / 旅行准备 / 项目安排"] --> B["输入校验"]
  B --> C["写入 Conversation 用户消息"]
  C --> D["构建上下文 AgentContext"]
  D --> E["注入记忆 + 最近会话历史"]
  E --> F["AgentExecutor 构建 ReAct 决策 prompt"]
  F --> G["LLM Thought: 判断下一步"]
  G --> H["LLM Action: 从可用工具中选择一个 Skill"]
  H --> I["Harness 包裹执行并推送流式事件"]
  I --> J["Observation: Skill 输出或错误"]
  J --> K{"LLM 选择 finish 或达到完成条件？"}
  K -->|否| F
  K -->|是| L["聚合结果"]
  L --> M["保存历史记录"]
  L --> N["保存日历事件"]
  L --> O["更新记忆"]
  L --> P["写入 Conversation assistant 消息"]
  L --> Q["前端消息流展示"]
```

对应到代码，主流程在 `AgentExecutor.run()` 里：

1. 校验输入不能为空。
2. 构建 `AgentContext`。
3. 调用 `createPlan()` 生成计划元数据；当前主要用于保留目标和文件导出语义。
4. 创建 `AgentHarness(10)`，最多允许 10 个被 Harness 包裹的工具步骤。
5. 检查 LLM 是否可用。
6. 进入 ReAct 循环：`AgentExecutor` 把当前状态、可用工具和上一轮 Observation 交给 LLM；LLM 返回 `thought` 和 `action`；后端校验 action 是否可用，然后 `AgentHarness` 执行对应 Skill 并写入 `Observation`。
7. 保存记忆、日历、历史记录。
8. 如果传入了 `conversationId`，把 assistant 消息和完整 `runResult` 写回会话。
9. 返回给前端；流式模式下还会持续推送步骤事件。

## 2. 项目的主要模块

```mermaid
flowchart LR
  UI["React 前端"] --> API["Express API"]
  API --> Conversation["ConversationStore"]
  API --> Executor["AgentExecutor"]
  Executor --> Context["Context Builder"]
  Executor --> Planner["Planner 元数据"]
  Executor --> Harness["AgentHarness"]
  Executor --> React["LLM ReAct Decision"]
  React --> Harness
  Harness --> Skills["Skills"]
  Skills --> LLM["LLM Provider"]
  Skills --> Local["本地规则"]
  Executor --> Memory["MemoryManager"]
  Executor --> Calendar["CalendarStore"]
  Executor --> History["RunHistoryStore"]
  Executor --> Files["FileWriterSkill"]
  Executor --> Stream["AgentRunEvent Stream"]
  API --> Templates["TemplateStore"]
  API --> Goals["GoalStore"]
  API --> Reviews["ReviewStore"]
  Stream --> UI
```

每个模块负责一件事：

- 前端负责输入、展示结果、编辑日历。
- API 负责接收请求、校验输入、可选鉴权、流式响应和会话路由。
- `ConversationStore` 负责会话列表、消息追加和会话详情。
- `AgentExecutor` 负责串起整个 Agent 流程。
- `Context Builder` 负责准备上下文。
- `Planner` 当前负责生成计划元数据；真正的下一步动作由 ReAct 决策 prompt 交给 LLM 选择。
- `AgentExecutor` 负责维护当前状态、计算可用工具、请求 LLM 选择 Action，并把工具结果写回状态。
- `Harness` 是执行控制层，负责限制步数、包裹 Skill 调用、统一错误处理，并在这个过程中产出可观测日志。
- `Skills` 负责具体能力。
- `MemoryManager` 负责记忆检索和保存。
- `TemplateStore` 负责场景模板。
- `GoalStore` 和 `ReviewStore` 负责长期目标与周期复盘。
- `CalendarStore`、`RunHistoryStore` 负责持久化。

## 3. 什么是 Prompt 工程

Prompt 工程不是简单写一句“请帮我规划”。在这个项目里，Prompt 工程有三个目标：

1. 告诉模型扮演什么角色。
2. 告诉模型必须遵守哪些规则。
3. 要求模型输出程序能解析的数据，并在解析失败时有清晰的重试或失败路径。

### 3.1 任务拆解 Prompt

相关文件：

- `apps/server/src/skills/taskDecomposer.skill.ts`
- `apps/server/src/skills/definitions/task-decomposer.skill.md`

任务拆解 Skill 会让模型做这件事：

> 把用户的自然语言目标拆成结构化任务数组。

它的 prompt 会要求模型：

- 只围绕用户输入拆任务。
- 不要输出空泛任务，比如“明确目标”“制定计划”。
- 每个任务都要有标题、描述、优先级、预计时间、依赖和标签；截止日期只有在用户给出明确日期或模型能合理推断时才出现。
- 如果用户提到了地点、时间、数量，要体现在任务里。
- 输出 JSON，不要输出随意文本。

流程如下：

```mermaid
flowchart TD
  A["用户输入"] --> B["任务拆解 Prompt"]
  B --> C["LLM 返回 JSON"]
  C --> D["normalizeTasks 清洗字段"]
  D --> E["PlannedTask[]"]
```

为什么要清洗字段？因为 LLM 可能输出不完整或格式不稳定。代码里会做这些处理：

- 没有标题时给默认标题。
- 优先级不合法时改成 `medium`。
- 预计时间限制在合理范围内。
- `dueDate` 如果存在，必须符合 `YYYY-MM-DD`；不合法时会被丢弃。
- 最多保留 10 个任务。

这就是工程里的安全垫。

### 3.2 JSON 输出和自动修复

相关文件：

- `apps/server/src/llm/openaiProvider.ts`

`generateJSON()` 会给模型追加格式要求：

- 只能返回合法 JSON。
- 不要解释。
- 不要 Markdown 代码块。
- 必须以 `[` 或 `{` 开头。
- 按 `schemaHint` 输出。

如果模型第一次没返回合法 JSON，代码会自动重试。

```mermaid
flowchart TD
  A["业务 Prompt"] --> B["追加 JSON 输出要求"]
  B --> C["调用 LLM"]
  C --> D{"能解析 JSON？"}
  D -->|能| E["返回结构化数据"]
  D -->|不能| F["低温度重试"]
  F --> C
  D -->|多次失败| G["抛出错误"]
```

这很重要，因为真实项目里不能假设模型永远听话。

### 3.3 建议生成 Prompt

相关文件：

- `apps/server/src/skills/recommendation.skill.ts`
- `apps/server/src/skills/definitions/recommendation.skill.md`

建议生成 Skill 不只看任务，还会看用户原始目标：

```ts
interface RecommendationInput {
  goal: string;
  tasks: PlannedTask[];
  events: CalendarEvent[];
}
```

这样做是因为任务拆解后的结构化数据可能丢掉一些语气和背景。比如用户说“我想轻松一点安排大阪旅行”，这个“轻松一点”可能不会完整出现在任务标题里，但原始输入里还在。

旅行场景里，Prompt 还特别限制模型不要乱编：

- 可以推荐候选城市、区域、景点类型。
- 但要说成“可候选”“可优先调研”。
- 不要编造实时价格。
- 不要编造营业时间。
- 不要编造签证政策。
- 不要编造交通班次。
- 不要编造具体网址。
- 涉及不稳定信息时，提醒用户二次核验。

```mermaid
flowchart TD
  A["用户原始目标"] --> D["推荐建议 Prompt"]
  B["任务列表"] --> D
  C["日历事件"] --> D
  D --> E["LLM 生成建议"]
  E --> F["过滤空字符串"]
  F --> G["最多保留 6 条建议"]
```

### 3.4 最终总结 Prompt

相关文件：

- `apps/server/src/agent/executor.ts`

`generateFinalAnswer()` 负责生成最后给用户看的总结。它会把这些信息交给模型：

- 用户原始输入。
- 拆出来的任务。
- 生成的建议。

它要求模型用 2-3 句话总结，并指出关键执行要点和风险。

## 4. 什么是上下文工程

上下文工程就是：在模型执行前，给它准备刚刚好的信息。

不是信息越多越好。信息太少，模型不知道背景；信息太多，模型容易混乱、成本也更高。

本项目的上下文对象叫 `AgentContext`。

相关文件：

- `apps/server/src/agent/context.ts`
- `apps/server/src/agent/types.ts`

```ts
export interface AgentContext {
  runId: string;
  input: string;
  now: string;
  memories: MemoryItem[];
  conversationHistory?: ConversationTurn[];
  conversationSummary?: string;
}
```

图示如下：

```mermaid
flowchart LR
  A["runId 本次运行 ID"] --> E["AgentContext"]
  B["input 用户原始输入"] --> E
  C["now 当前时间"] --> E
  D["memories 相关记忆"] --> E
  E --> F["任务拆解 Skill"]
  E --> G["网址整理 Skill"]
  E --> H["结果聚合"]
```

这些字段分别解决不同问题：

- `runId`：让文件、历史、日历能关联到同一次运行。
- `input`：保留用户原始意图。
- `now`：让日程规划知道当前时间。
- `memories`：让 Agent 能记住用户偏好。
- `conversationHistory`：注入最近会话消息，让后续输入可以引用前文。
- `conversationSummary`：长会话时提供早期消息摘要，避免 prompt 过长。

### 4.1 会话上下文如何注入

相关文件：

- `apps/server/src/storage/conversationStore.ts`
- `apps/server/src/agent/context.ts`

流程：

```mermaid
flowchart TD
  A["用户发送消息"] --> B["ConversationStore.addUserMessage"]
  B --> C["buildContext(input, memoryManager, conversationStore, conversationId)"]
  C --> D["读取最近 24 条消息"]
  D --> E["保留最近 12 条作为 conversationHistory"]
  D --> F["更早消息生成轻量 summary"]
  E --> G["AgentContext"]
  F --> G
  G --> H["ReAct 决策 Prompt"]
```

assistant 消息不会把完整 `runResult` 原样塞进 prompt，而是注入最终回答和任务/日历/建议数量摘要。这样可以保留上下文，又避免把大对象塞满 prompt。

## 5. 原始输入和结构化数据为什么都要保留

一个常见误区是：只要把用户输入拆成任务，就不需要原始输入了。

这个项目没有这么做。它会同时保留：

- 原始输入 `input`
- 结构化任务 `tasks`
- 日历事件 `calendarEvents`

```mermaid
flowchart TD
  A["用户原始输入"] --> B["AgentContext"]
  B --> C["任务拆解"]
  C --> D["PlannedTask[]"]
  D --> E["CalendarEvent[]"]
  A --> F["RecommendationInput.goal"]
  D --> F
  E --> F
  F --> G["更贴近用户真实需求的建议"]
```

原始输入保留语义和语气，结构化数据方便程序处理。两者一起用，效果更稳。

## 6. Agent Harness 和经典 ReAct Loop 是什么

相关文件：

- `apps/server/src/agent/harness.ts`
- `apps/server/src/agent/types.ts`

经典 ReAct 的核心是：

```text
Thought -> Action -> Observation -> Thought -> Action -> Observation ...
```

也就是 Agent 先说明“我为什么要做下一步”，再调用一个动作，最后观察动作结果。

当前项目采用经典 ReAct loop：下一步 Action 由 LLM 在每轮从可用工具里选择，代码负责校验可选范围、执行工具、保存 Observation 和维护结构化状态。

Harness 可以理解成 Agent 的“执行外壳”或“运行约束层”。它不负责思考，也不负责决定下一步业务动作；它负责安全地执行每一个被模型选中的 Skill 调用。

它做这些事：

1. 限制最多执行几步。
2. 在执行前创建 step log。
3. 包裹 `skill.execute(input)`。
4. 成功时把输出摘要写成 `Observation`。
5. 失败时把错误信息写成 `Observation`。
6. 记录开始时间、结束时间、成功或失败状态。
7. 推送 `step` 和 `observation` 事件给流式响应。
8. 在步骤边界检查 abort signal，支持用户停止运行。

因此，更严谨的说法是：**Harness 的核心职责是执行约束和调用管理；日志是它为了可观测性在执行过程中产生的 trace。**

```mermaid
sequenceDiagram
  participant Executor as AgentExecutor
  participant Harness as AgentHarness
  participant Skill as Skill.execute()
  participant Log as Step Log

  Executor->>Harness: runStep(name, skill, input, LLM thought/action)
  Harness->>Log: 创建 step log，写入 LLM 选择的 Thought/Action
  Harness-->>Executor: onEvent(step)
  Harness->>Skill: 执行 Skill
  alt 成功
    Skill-->>Harness: output
    Harness->>Log: Observation + status=success
    Harness-->>Executor: onEvent(observation)
    Harness-->>Executor: output
  else 失败
    Skill-->>Harness: error
    Harness->>Log: Observation=错误信息 + status=error
    Harness-->>Executor: onEvent(observation)
    Harness-->>Executor: throw error
  end
```

### 6.1 最大步数限制

`AgentExecutor` 中创建：

```ts
const harness = new AgentHarness(autonomousStepLimit);
```

`AgentHarness` 内部会检查：

```ts
if (this.steps.length >= this.maxSteps) {
  throw new Error(`Agent exceeded max steps: ${this.maxSteps}`);
}
```

这避免 Agent 无限执行。

### 6.2 Step Log 中哪些字段是谁产生的

每一步都会生成日志，结构类似：

```ts
{
  id: "1",
  name: "任务拆解",
  skillName: "task_decomposer",
  status: "running",
  thought: "我需要先理解用户目标，并把自然语言需求变成可执行任务。",
  action: "task_decomposer",
  inputSummary: "...",
  startedAt: "...",
  usedLLM: true
}
```

执行结束后，会补上：

- `status`
- `outputSummary`
- `observation`
- `error`
- `endedAt`

字段来源要区分清楚：

- `thought`：由 LLM 在 ReAct 决策轮生成，不是 Harness 自己思考出来的。
- `action`：由 LLM 从当前可用工具中选择，通常对应要调用的 Skill。
- `observation`：由 `AgentHarness` 根据 Skill 执行结果或错误生成。
- `status` / `startedAt` / `endedAt`：由 `AgentHarness` 管理。
- `usedLLM`：由 `AgentExecutor` 判断后传入 Harness。

前端的 ReAct 执行日志就是从这些字段来的。

### 6.3 流式事件协议

相关文件：

- `apps/server/src/routes/agent.routes.ts`
- `apps/server/src/agent/types.ts`
- `apps/web/src/lib/api.ts`

前端默认通过 `fetch` + `ReadableStream` 调用：

```ts
POST /api/agent/run
body: {
  input: string;
  conversationId?: string;
  stream: true;
  options?: AgentOptions;
}
```

后端返回 `text/event-stream`：

```ts
type AgentRunEvent =
  | { type: "step"; step: AgentStepLog }
  | { type: "observation"; step: AgentStepLog }
  | { type: "partial"; result: Partial<HarnessRunResult> }
  | { type: "done"; result: HarnessRunResult; conversationId?: string }
  | { type: "error"; message: string; partialResult?: Partial<HarnessRunResult> };
```

这里使用 `fetch` 而不是原生 `EventSource`，因为当前接口是 `POST /api/agent/run`，而 `EventSource` 只支持 GET。

### 6.4 中断逻辑和一次修复

前端停止按钮会调用 `AbortController.abort()`。后端把 abort signal 传给 `AgentHarness`，Harness 在步骤开始和工具执行后检查：

```ts
if (this.options.signal?.aborted) {
  const error = new Error("用户中断了 Agent 运行。");
  error.name = "AbortError";
  throw error;
}
```

曾经的问题是：路由层用 `req.on("close")` 来判断用户取消，但正常 POST 请求也可能触发 close，导致后端误判为用户中断。现在改为：

- `req.on("aborted")`：请求确实被客户端中止。
- `res.on("close")`：响应未完成却关闭时才中断。
- `finished` 标记：正常完成后不会再触发 abort。

这避免了“发出对话后自动出现用户中断了 Agent 运行”的问题。

### 6.5 ReAct 决策 prompt 如何约束工具

相关代码在 `apps/server/src/agent/executor.ts` 的 `decideNextAction()`。

每一轮 prompt 会给模型这些信息：

- 用户原始输入。
- 当前时间和相关记忆。
- 最近会话历史和会话摘要。
- 当前结构化状态，例如任务数量、是否已有最终回答、文件数量。
- 上一轮 Observation。
- 当前可用工具列表。
- `allowedActions` 白名单。

模型必须返回 JSON：

```json
{
  "thought": "说明为什么选择下一步",
  "action": "task_decomposer",
  "reason": "可选的简短说明"
}
```

`AgentExecutor` 会校验 action 是否在 `allowedActions` 里。如果模型返回格式错误或选了不可用工具，代码会使用保守的确定性 fallback 选择下一步，以降低一次决策输出不稳导致运行中断的概率。

## 7. LLM 步骤和本地步骤怎么区分

不是所有步骤都应该交给 LLM。

```mermaid
flowchart LR
  A["模糊语义任务"] --> B["LLM"]
  B --> C["任务拆解 / 建议 / 总结"]
  D["确定性任务"] --> E["本地代码"]
  E --> F["排序 / 日历保存 / 文件写入 / 历史记录"]
  C --> G["最终结果"]
  F --> G
```

适合 LLM 的任务：

- 理解自然语言。
- 拆解目标。
- 给出场景化建议。
- 写自然语言总结。

适合本地代码的任务：

- 排序。
- 保存日历。
- 写文件。
- 保存历史记录。
- 删除或编辑日历事件。

这样设计的好处是：LLM 负责理解、决策和生成，本地代码负责可控的状态变更。

## 8. 记忆系统怎么工作

相关文件：

- `apps/server/src/memory/memoryManager.ts`
- `apps/server/src/memory/memoryStore.ts`
- `apps/server/src/routes/memory.routes.ts`

记忆系统分两层：

- `MemoryStore`：负责读写 `data/memories.json`。
- `MemoryManager`：负责检索、更新和推断记忆。

```mermaid
flowchart TD
  A["用户输入"] --> B["retrieveRelevant(input)"]
  C["data/memories.json"] --> B
  B --> D["相关记忆"]
  D --> E["AgentContext.memories"]
  E --> F["Prompt 使用这些记忆"]
  A --> G["maybeInferMemories(input)"]
  G --> H{"发现稳定偏好？"}
  H -->|是| I["写入低置信度记忆"]
  H -->|否| J["不写入"]
  I --> C
```

### 8.1 MemoryItem

记忆的数据结构是：

```ts
export interface MemoryItem {
  id: string;
  type: "preference" | "habit" | "constraint" | "profile" | "project" | "other";
  key: string;
  value: string;
  confidence: number;
  source: "user_explicit" | "agent_inferred";
  status: "pending" | "active" | "archived";
  layer: "working" | "long";
  hitCount: number;
  lastHitAt?: string;
  createdAt: string;
  updatedAt: string;
}
```

重点字段：

- `confidence`：可信度。
- `source`：记忆来源。
- `status`：pending 需要用户确认，active 才能进入后续上下文，archived 不再参与检索。
- `layer`：working 表示偏短期的上下文增强，long 表示长期偏好或约束。
- `hitCount` / `lastHitAt`：用于观察记忆是否真的被使用。

如果是 Agent 自己推断出来的记忆，代码会设置：

```ts
confidence: 0.45,
source: "agent_inferred",
status: "pending",
layer: "long",
hitCount: 0
```

这表示：Agent 可以猜测用户偏好，但不能把猜测当成确定事实。

### 8.2 相关记忆检索

`retrieveRelevant(input)` 会：

1. 读取所有记忆。
2. 只保留 `status=active` 的记忆。
3. 如果 LLM 可用且 `MEMORY_LLM_TOPK` 没有设为 `false`，让模型从候选记忆中选 Top-K。
4. 如果模型不可用、返回错误或被禁用，降级为关键词打分。
5. 命中的记忆会增加 `hitCount` 并更新 `lastHitAt`。

这个方案暂不引入向量数据库，但已经比子串匹配更稳：模型能判断语义相关性，工程层仍保留可预测 fallback。

### 8.3 为什么记忆要 pending

自动推断记忆很强，但也容易伤害信任。比如用户一次性说“这周晚上学习”，不代表长期都偏好晚上学习。所以 v2 的规则是：

- 用户主动新增的记忆默认 active。
- Agent 推断出的记忆默认 pending。
- pending 记忆只展示给用户确认，不参与后续上下文。
- 用户确认后变为 active。
- 用户也可以编辑、归档或删除。

前端对应页面是 `工作台 -> 记忆`。

## 9. 计划资产层：模板、目标、复盘和文件

v2.x 额外增加了一组“不直接参与 ReAct loop，但会支撑长期使用”的资产层：

- `TemplateStore`：场景模板，内置学习计划、旅行准备、项目冲刺、周期复盘。
- `GoalStore`：长期目标，记录目标周期、状态、标签和下次复盘时间。
- `ReviewStore`：周期复盘，记录 wins、blockers、nextActions。
- `files.routes.ts`：支持生成文件列表、单文件删除和按天数清理。
- `calendar.routes.ts`：支持 ICS 导入导出。

它们的意义是让 Agent 结果不只停留在一次回答里，而是能逐步沉淀为可维护的个人计划资产。

## 10. Skills 是什么

Skill 可以理解成 Agent 的一个能力模块。

比如：

- 任务拆解是一个 Skill。
- 优先级排序是一个 Skill。
- 日历规划是一个 Skill。
- 网址整理是一个 Skill。
- 文件生成是一个 Skill。

项目里的 Skill 分成两部分：

1. Markdown 定义文件：描述这个 Skill 应该做什么。
2. TypeScript runtime：真正执行这个 Skill。

```mermaid
flowchart LR
  A["*.skill.md"] --> B["loadMarkdownSkill"]
  B --> C["definition.prompt"]
  D["*.skill.ts"] --> E["execute(input)"]
  C --> F["withMarkdownDefinition"]
  E --> F
  F --> G["统一 Skill 对象"]
  G --> H["AgentHarness.runStep"]
```

### 10.1 Markdown Skill

相关目录：

- `apps/server/src/skills/definitions/`

一个 Markdown Skill 大概长这样：

```md
---
name: task_decomposer
title: 任务拆解 Skill
description: 将用户的自然语言目标拆解成可执行任务。
input: AgentContext
output: PlannedTask[]
---

## 规则

- 每个任务必须包含 title、description、priority、estimatedMinutes、dependencies 和 tags；有明确截止日期时再包含 dueDate。
```

frontmatter 里的字段是机器可读信息：

- `name`
- `title`
- `description`
- `input`
- `output`

正文是给 LLM 和开发者看的规则。

### 10.2 Runtime Skill

相关文件：

- `apps/server/src/skills/*.skill.ts`

所有 runtime Skill 都符合这个接口：

```ts
export interface Skill<I, O> {
  name: string;
  description: string;
  execute(input: I): Promise<O>;
}
```

这表示不同 Skill 可以有不同输入输出，但都能被 Harness 统一执行。

### 10.3 Markdown 和 Runtime 怎么绑定

相关文件：

- `apps/server/src/skills/markdownSkill.ts`
- `apps/server/src/skills/index.ts`

绑定方式是：

```ts
export const taskDecomposerSkill = withMarkdownDefinition(taskDecomposerRuntime, "task-decomposer.skill.md");
```

绑定后，一个通过 `withMarkdownDefinition()` 包装的 Skill 同时拥有：

- Markdown 里的 `name`
- Markdown 里的 `description`
- Markdown 正文 `definition.prompt`
- TypeScript 的 `execute()`

所以 Markdown 记录“做什么”和行为约束，TypeScript runtime 决定“怎么执行”和如何处理输出。

## 11. 如果新增一个 Skill，要做什么

```mermaid
flowchart TD
  A["新增 xxx.skill.md"] --> B["写 name/title/input/output/规则"]
  C["新增 xxx.skill.ts"] --> D["实现 execute(input)"]
  B --> E["在 skills/index.ts 绑定"]
  D --> E
  E --> F["在 Executor 工具 runtime 中加入输入、输出和可用条件"]
  F --> G["LLM 选择 Action，Harness 执行并产出 observation/error trace"]
```

这就是项目的扩展点。新增能力时，通常不需要重写整个 Agent，但需要把新 Skill 注册到工具 runtime，并明确它何时可用、如何写回状态、失败后如何处理。

## 12. 结果结构是什么

一次 Agent 运行最后返回 `HarnessRunResult`。

相关文件：

- `apps/server/src/agent/types.ts`

```ts
export interface HarnessRunResult {
  runId: string;
  agentPattern: "react";
  steps: AgentStepLog[];
  finalAnswer: string;
  tasks: PlannedTask[];
  calendarEvents: CalendarEvent[];
  urls: UrlReference[];
  files: GeneratedFile[];
  memoriesUsed: MemoryItem[];
  recommendations: string[];
  llmStatus?: LLMStatus;
}
```

图示如下：

```mermaid
flowchart TD
  A["HarnessRunResult"] --> B["finalAnswer 最终回答"]
  A --> C["tasks 任务列表"]
  A --> D["calendarEvents 日历事件"]
  A --> E["urls 网址"]
  A --> F["files 生成文件"]
  A --> G["memoriesUsed 使用过的记忆"]
  A --> H["steps 执行日志"]
  A --> I["llmStatus 模型状态"]
```

`agentPattern: "react"` 表示返回结果包含经典 ReAct 的 Thought / Action / Observation 执行轨迹。

这说明项目返回的不只是一段文字，而是结构化结果。某些数组可能为空，例如模型没有选择文件生成工具、关闭了日历生成，或 LLM 不可用导致完整 ReAct 循环没有启动。

## 13. 降级策略

相关文件：

- `apps/server/src/llm/index.ts`
- `apps/server/src/llm/openaiProvider.ts`

项目会根据是否配置 `LLM_API_KEY` 选择 Provider：

- 有配置时使用 `OpenAICompatibleProvider`。
- 没配置时使用 `MockProvider`。

运行前还会调用 `testLLMConnection()` 检查模型是否真的可用。

```mermaid
flowchart TD
  A["开始运行"] --> B["testLLMConnection"]
  B --> C{"LLM 可用？"}
  C -->|可用| D["使用 OpenAICompatibleProvider"]
  C -->|不可用| E["使用 MockProvider 或进入错误/局部降级路径"]
  D --> F["步骤标记 usedLLM=true"]
  E --> G["记录未连接状态或局部降级结果"]
  F --> H["返回 llmStatus"]
  G --> H
```

这里要注意一个边界：后端服务不会因为没有 API Key 就无法启动，但经典 ReAct 的工具选择和部分业务工具依赖 LLM。`prioritySorter`、`calendarPlanner`、`fileWriter` 等工具本身是本地确定性逻辑；`taskDecomposer`、`urlCollector`、`recommendation` 和 `finalAnswer` 属于 LLM 依赖更强的工具。`MockProvider` 提供的是 provider 层占位能力，不等于每个业务 Skill 都已经有完整本地 fallback。

## 14. 前端如何使用 Agent 输出

相关文件：

- `apps/web/src/App.tsx`
- `apps/web/src/pages/Dashboard.tsx`
- `apps/web/src/components/CalendarView.tsx`
- `apps/web/src/lib/api.ts`

v2.0 的前端分成左侧导航和主工作区：

- 左侧展示 LLM 连接状态小框、新对话、会话列表、工作台二级菜单和历史记录。
- `工作台 -> 对话` 展示消息流、结构化结果、执行日志和底部输入框。
- `工作台 -> 日历` 单独展示可编辑日历。
- 历史记录页展示过去的 Run 快照。

```mermaid
flowchart LR
  A["AgentRunEvent Stream"] --> B["runAgentStream"]
  B --> C["Dashboard 消息流"]
  C --> D["运行中步骤"]
  C --> E["assistant runResult"]
  E --> F["任务列表 / URL / 文件 / 执行日志"]
  G["左侧日历入口"] --> H["CalendarView"]
  H --> I["/api/calendar"]
  I --> J["CalendarStore"]
```

日历组件 `CalendarView` 可以：

- 读取 `/api/calendar`。
- 新增事件。
- 编辑事件。
- 删除事件。
- 展示历史记录中的只读日历快照。

### 14.1 为什么把 LLM 状态和日历收起来

早期 v2.0 界面把 LLM 状态和周日历放在对话页顶部，首屏空间被占用太多。现在调整为：

- LLM 状态放到左侧小框，只保留连接/未连接、模型、延迟和刷新按钮。
- 日历放到 `工作台 -> 日历`，需要调整日程时再进入。
- 对话页把首屏还给消息流，让用户发起规划和阅读结果更轻。

## 15. Docker、CI、格式化和 SQLite 迁移

v2 的工程化入口：

- `Dockerfile` 和 `docker-compose.yml`：容器内构建前后端，后端托管 `apps/web/dist` 静态文件。
- `.github/workflows/ci.yml`：在 push / PR 时执行 `npm ci`、format check、lint、typecheck、test、build。
- `eslint.config.js` 和 `.prettierrc`：统一 TypeScript、React Hooks 和格式化规则。
- `apps/server/src/storage/sqliteMigration.ts`：读取 JSON 数据，生成 `data/sqlite/schema.sql` 和 `seed.sql`。

当前默认 runtime 仍是 JSON store。SQLite 部分完成的是迁移出口，不强制引入 SQLite driver；这样可以保持现有本地部署稳定，也给后续切换数据库留出路径。

## 16. 为什么这算一个 Agent 项目

这个项目体现 Agent 的地方在于：

1. 它不是只调用一次模型，而是把目标拆成多个步骤。
2. 它会构建上下文，把输入、时间和记忆组合起来。
3. 它采用经典 ReAct 的 Thought/Action/Observation 循环暴露执行轨迹，并用 `allowedActions` 约束模型可选动作。
4. 它有 Skills，把复杂能力拆成可组合模块。
5. 它会区分 LLM 任务和本地确定性任务。
6. 它有记忆系统，可以跨运行保留用户偏好。
7. 它能把结果保存成历史记录，并在对应工具被调用时保存日历事件和生成文件。
8. 它有防幻觉约束，尤其是旅行推荐这种容易编造事实的场景。

可以把它理解成一个小型 Agent Runtime：

```mermaid
flowchart TD
  A["Prompt"] --> G["Agent Runtime"]
  B["Context"] --> G
  C["Harness"] --> G
  D["Skills"] --> G
  E["Memory"] --> G
  F["Storage"] --> G
  G --> H["可用的任务规划结果"]
```

每一层的职责是：

- Prompt：让模型按规则生成内容。
- Context：给模型和 Skill 提供必要背景。
- Harness：限制执行边界、包裹 Skill 调用、统一错误处理，并产生可观测 trace。
- Skills：把复杂任务拆成多个能力。
- Memory：保存和检索用户偏好。
- Storage：把结果保存成可以继续使用的数据。
