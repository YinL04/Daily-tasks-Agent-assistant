...
- ReAct Harness、ConversationStore、上下文注入、流式事件、中断行为、记忆分层、模板、长期目标和周期复盘测试，当前质量文档记录为 12 个测试全部通过。
...
| 记忆 | 已支持 pending/active/archived、确认/编辑/归档/删除、LLM Top-K 筛选 | 后续可引入向量库和更强隐私策略     |
| 安全 | 已增加输入限制、路径校验、健康端点脱敏、可选 Token 和基础限流       | 后续补更完整安全矩阵               |
| 测试 | 已覆盖后端核心链路，缺少前端/E2E 自动化                             | 继续补 API、前端、E2E              |
| 工程 | 已有 lint/format/CI/Docker/SQLite SQL 导出                          | 后续可接入 SQLite runtime driver   |
...
- 不强制切换到 SQLite runtime，不引入向量数据库和复杂多模型编排；当前仅提供 SQLite migration SQL。
...
- SQLite runtime 迁移不阻塞 v2；当前已提供 `npm run migrate:sqlite -w apps/server` 导出 schema/seed SQL。
...
当前 v2 已完成后端核心测试 12 个，覆盖 Harness 成功/失败/最大步数、ReAct trace、ConversationStore、上下文注入、流式事件、中断行为、记忆分层、模板、长期目标和周期复盘。前端与 E2E 自动化仍属于后续补强项。
...
| M6   | 第 6 周 | CI、lint、Docker、SQLite SQL、模板、目标、复盘 | CI 绿灯，部署说明可执行            |
...
| JSON 存储并发写入冲突        | 中   | 数据损坏           | 原子写入、写队列；已保留 SQLite SQL 迁移路线 |
...
| Q4   | SQLite 是否纳入 v2？       | 已纳入 SQL 迁移出口，runtime driver 后续接入 |
